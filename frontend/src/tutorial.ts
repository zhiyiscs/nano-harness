...
export const TUTORIAL_STAGES: TutorialStage[] = [
  ...
  },
  {
    ...
    moduleName: "3. Add a notebook",
    what: "Memory is the temporary notebook. It keeps search results and intermediate findings around for later steps.",
    why: "A harness works one small step at a time. Without a notebook, the next step may forget what the previous step found.",
    runHint: "After it runs, check the notebook section in the trace. If the search result is still there, memory helped.",
    problem: "If every step starts from scratch, the workflow behaves like it is forgetful.",
    fix: "Memory lets later steps reuse what earlier steps discovered.",
    tryThis: "Hit \"See the trace\" to check whether the search result sticks around this time.",
  ...
  },
  {
    ...
    what: "Context Builder prepares the material the model gets to see right now. It picks from the task and notebook and turns that into a short prompt.",
    why: "The model only sees the current input. It does not automatically read the notebook sitting beside it.",
    runHint: "After it runs, look for the highlighted \"What the model saw\" row and expand it. That is the exact input the model received for that step.",
    problem: "Information can exist somewhere without being in front of the model when it makes a decision.",
    fix: "Context Builder turns the question, notes, and evidence into the input for the current step.",
    tryThis: "Hit \"See the trace\", then open the highlighted row to compare: what is in memory versus what the model actually receives.",
